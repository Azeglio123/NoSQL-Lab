...
Next, important secrets or keys are imported from the .env file. The configuration of the database and APIs as well as a folder to store images is set up. The Version 5 of the SpaceX API is only working for fetching the launches, the remaining endpoints use version 4.
...
Most launches carry a single core and one or two payloads. This is expected since the majority of SpaceX missions use a single Falcon 9 booster. Falcon Heavy missions, which use three cores, would show a higher `core_count` [@uri_5_2023]. The `cores` array contains embedded documents with landing information, making it a key source for the reuse and landing analyses later.
...
58 cores have a null `landpad` field. As the first 10 results show, these are all early missions (2006–2013) where `landing_attempt` is `false` and no recovery was attempted. This is expected, as SpaceX did not begin attempting booster landings until late 2015 [@wall_wow_2015]. These cores are not erroneous and do not need to be cleaned. They simply represent missions where landing infrastructure was not yet available. The analysis pipelines in sections 6.3 and 6.6 already handle this by filtering on `landing_attempt: true`.
...
The SpaceX API stores rocket and launchpad as string IDs rather than embedded documents. Using both patterns revealed the benefits and trade-offs. While `cores` are embedded directly within launch documents for fast access, whereas `rocket` and `launchpad` are stored as string references to separate collections requiring `$lookup` joins. Embedding reduces query complexity but duplicates data, while referencing keeps data normalized but increases aggregation complexity.
...
As a fun fact, most of the SpaceX endpoints include images of the entities. Below, the url of a Falcon Heavy image from the raw collection is loaded and displayed in Figure 8.

```{python}
#| label: fig-rocket-image
#| fig-cap: "Image of Falcon Heavy Rocket"
#| fig-width: 90%
...
# Get Falcon Heavy rocket image from raw collection using aggregation
pipeline = [
    {"$match": {"name": "Falcon Heavy"}},
    {"$project": {"_id": 1, "flickr_images": 1}}
]

result = list(rockets_raw.aggregate(pipeline))
image_url = random.choice(result[0]["flickr_images"])

# Save image to file
response = requests.get(image_url)
img = PILImage.open(BytesIO(response.content))
img.save("Personal_project_files/images/falcon_heavy.png")

# Display from local file using matplotlib (works in PDF rendering)
fig, ax = plt.subplots(figsize=(10, 6))
ax.imshow(mpimg.imread("Personal_project_files/images/falcon_heavy.png"))
ax.axis("off")
plt.tight_layout()
plt.show()
```
