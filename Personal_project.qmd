...
```{python}
#| label: most-active-launchpads

# Analysis 4: Launchpad performance
pipeline_launchpad_perf = [
    {
        "$match": {
    ...
    },
    {
        "$group": {
            ...
            "total_launches": {"$sum": 1},
            "successful": {"$sum": {"$cond": ["$success", 1, 0]}},
            "years_active": {"$addToSet": "$launch_year"}
        }
    },
    {
        "$lookup": {
            "from": "launchpads_stage",
            "localField": "_id",
            "foreignField": "_id",
            "as": "pad_info"
        }
    ...
    },
    {
        "$project": {
            "_id": 0,
            "launchpad": "$pad_info.full_name",
            "location": "$pad_info.location",
            "total_launches": 1,
            "successful": 1,
            "success_rate_pct": 1,
            "years_span": 1
...
results = list(launches_stage.aggregate(pipeline_launchpad_perf))
df_pads = pd.DataFrame(results)
df_pads
```
