...
# to render labels correctly: use a regular ASCII hyphen
...
The horizontal bar chart reveals that Falcon 9 dominates SpaceX's launch history with 178 missions and an exceptional 98.9% success rate. The rocket has only 2 failures across its entire operational life. Falcon Heavy, while still in its early phase with just 3 launches, holds a perfect 100% record. Falcon 1, SpaceX's first orbital rocket, succeeded only 2 out of 5 times, reflecting the steep learning curve of the company's early years. The colour coding, green for ≥ 90%, orange for ≥ 70%, red below, makes it immediately clear that SpaceX's current fleet is extraordinarily reliable.
...
The stacked bar chart shows that CCSFS SLC 40 in Florida is SpaceX's workhorse facility with 99 launches and a 98% success rate. Its 2 failures are the only blemishes across all active US-mainland pads. KSC LC 39A and VAFB SLC 4E both maintain flawless 100% records with 55 and 27 launches respectively. Kwajalein Atoll stands out as the sole non-continental site with a low 40% success rate across 5 early Falcon 1 missions. VAFB SLC 3W (retired, zero launches) and STLS (under construction) confirm that SpaceX's operational launch capacity is currently concentrated across three active Florida and California sites.
...
The dual-axis chart tells a compelling growth story. Launch volume increased slowly from 1–2 missions per year to 43 in 2022, showing near-exponential growth from 2017 onward. The success rate line mirrors this maturation: early years were volatile (0% in 2006–2007, 50% in 2008), but from 2017 onward SpaceX achieved a perfect or near-perfect success rate every year. The two dips in 2015 and 2016 correspond to the CRS-7 in-flight breakup and the Amos-6 pad explosion respectively — both pivotal incidents that led to design improvements. The combination of rising cadence and sustained reliability from 2017 onward underscores SpaceX's transition from an experimental startup to a mature, high-tempo launch provider.
...
The bar-and-line chart reveals that the majority of cores flew only once. These include early expendable missions and newer cores that have not yet been reused. As the number of flights increases, the count of cores drops sharply, but the average landing successes per core rises steeply, reaching 11.5 for the most-reused bucket. This confirms that reused boosters are not only surviving multiple missions but consistently landing successfully, validating SpaceX's reusability model. The 6 cores in the 8+ bucket represent the fleet's most battle-tested hardware and are central to SpaceX's cost-reduction strategy.
...
The bar chart shows that Falcon 9 has a landing fail rate of only 7.2% across 152 attempts. Meaning 141 out of 152 boosters were successfully recovered. Falcon Heavy, with just 9 landing attempts, has a notably higher fail rate of 22.2%. The higher Falcon Heavy rate is partly explained by the complexity of recovering three boosters per mission and the smaller sample size. The colour coding (green for Falcon 9, red for Falcon Heavy) draws attention to this disparity. Nevertheless, both rates are impressive given that routine orbital-class booster recovery was considered impractical before SpaceX pioneered it.