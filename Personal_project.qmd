...
# group by launchpad and collect launch stats and active years
group = {"$group": {
    "_id": "$launchpad",
    "total_launches": {"$sum": 1},
    "successful": {"$sum": {"$cond": ["$success", 1, 0]}},
...
# join launchpad details from launchpads_stage by id
lookup = {"$lookup": {
...
# flatten pad_info array into a single object
unwind = {"$unwind": "$pad_info"}

# compute success rate and number of distinct years active
...
# clean up output fields and rename pad name and location
project = {"$project": {
    "_id": 0,
...
pipeline = [match, group, lookup, unwind, add_fields, project, sort]
cursor = launches_stage.aggregate(pipeline)
pd.DataFrame(cursor)
```
